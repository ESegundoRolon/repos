...
class Productos {
    ...
    public function productosDestacados($pagina=null,$resultados_x_pagina=null){
        if(!$pagina){
            $pagina = 1;
        }
        if($resultados_x_pagina){
    ...
    //crear producto
    public function crear($denominacion,$descripcion,$codigo,$precio){
        $datos["denominacion"] = $denominacion;
        $datos["descripcion"] = $descripcion;
        $datos["codigo"] = $codigo;
        $datos["precio"] = $precio;
        
        return $this->mysql->insertar("productos",$datos);
    }
    //simula la devolucion de json por parte de un webservice
    public function productosJson(){
        $datos = $this->mysql->consulta("SELECT SQL_CALC_FOUND_ROWS * FROM productos");
